...
public static class RomajiHelper
{
    ...
    public static List<ConvertedLine> ToRomaji(string text, bool isAutoVariant = false, float chineseRate = 1f)
    {
        ...
        var convertedText = new List<(ConvertedLine Item, int Index)>();

        Parallel.For(0, lineTextList.Length, (index) =>
        {
            var line = lineTextList[index];

            var convertedLine = new ConvertedLine();

            if (IsChinese(line, chineseRate)) return;
            ...
            convertedText.Add((convertedLine, index));
        });
        ...
        var result = convertedText.OrderBy(p => p.Index).Select(p => p.Item).ToList();
